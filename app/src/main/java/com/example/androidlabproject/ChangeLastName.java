...
public class ChangeLastName extends AppCompatActivity {
    ...
    public void changeLastname(View view) {
        ...
        if(newUsernameStr.isEmpty()){
        ...
        else if (newUsernameStr.equalsIgnoreCase(getLastName())){
            ...
            DataBaseHelper dataBaseHelper=new DataBaseHelper(this,"androidLabDB",null,1);
            builder.setView(dialogView2).setPositiveButton("OK", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialogInterface, int i) {
                    dataBaseHelper.updateLastName(new User(getIntent().getStringExtra("email"),getFirstName(),newUsernameStr,getPassword()));
                    finish();
                }
            });
            errorDialog = builder.create();
            errorTextView.setText("Last Name changed ..");
            errorDialog.show();
        }
    }
